...
export function Select({ className, wrapperClassName, children, value, onChange, disabled, ...props }) {
  ...
  const handleOpen = () => {
    ...
    if (!open && ref.current) {
      ...
      setDropUp(rect.bottom + 240 > window.innerHeight && rect.top > 240)
    }
    setOpen(v => !v)
  }
  ...
  return (
    <div ref={ref} className={cn('relative', wrapperClassName ?? 'w-full')}>
      ...
      {open && (
        <div className={cn(
          'absolute z-50 w-full min-w-[8rem] overflow-auto rounded-xl border border-border/60 bg-card shadow-2xl shadow-black/10 animate-dropdown-in',
          'max-h-60 py-1',
