...
class ClientCDRView(APIView):
    ...
    def _filtered_qs(self, request, tenant):
        ...
        # Voicemail: last_app must confirm voicemail actually ran.
        _vm_Q = (
    ...
    def _summary(self, request, tenant):
        ...
        _FAILED_CAUSES = (
            ...
            'INVALID_CALL_REFERENCE', 'INCOMPATIBLE_DESTINATION', 'INTERWORKING',
            'CRASH', 'SYSTEM_SHUTDOWN', 'LOSE_RACE', 'MANAGER_REQUEST',
            'USER_CHALLENGE', 'MEDIA_TIMEOUT', 'PICKED_OFF', 'PROGRESS_TIMEOUT', 'GATEWAY_DOWN', 'NORMAL_TEMPORARY_FAILURE',
        )
...
class ClientFaxFileView(APIView):
    ...
    def get(self, request, tenant_uuid):
        ...
        if fax_ids:
            qs = qs.filter(fax_id__in=fax_ids)
        ...
        while current <= local_end:
            ...
            if b:
                ...
                results.append({
                    ...
                    'total_duration': 0,
                    'total_billsec': 0,
                    'outgoing': {'total': 0, 'answered': 0, 'no_answer': 0, 'failed': 0, 'busy': 0},
