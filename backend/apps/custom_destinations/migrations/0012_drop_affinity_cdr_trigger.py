"""
Drop the CDR-driven affinity trigger.

Affinity is now captured live from CHANNEL_OUTGOING in esl_listener (see
backend/esl/management/commands/esl_listener.py: _maybe_capture_outbound_affinity).
The CDR path was producing bad rows (e.g. PSTN caller_id_number landing in
extension_number) and firing only after hangup. Live capture happens the moment
an extension dials out and only records real internal extension numbers.
"""
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('custom_destinations', '0011_affinity_last_write_wins'),
    ]
